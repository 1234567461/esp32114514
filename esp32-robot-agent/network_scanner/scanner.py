"""
自定义网络扫描器 —— 纯 Python socket 实现，不依赖 nmap/masscan 等任何外部工具。
支持: 主机发现(ARP+ICMP)、TCP 端口扫描(connect)、服务识别(banner grab)。
仅用于授权的自有网络资产盘点。
"""
import socket
import struct
...
class NetworkScanner:
    ...
    def tcp_port_scan(self, ip: str, port: int) -> Optional[Dict]:
        """对单个端口做 TCP connect 扫描 + banner grab"""
    ...
    def host_alive_ping(self, ip: str) -> bool:
        """自定义 ICMP echo（ping），构造原始 ICMP 报文"""
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_ICMP)
        except PermissionError:
            # 非 root 时退化为 TCP ping（尝试连接一个常用端口）
        ...
        header = struct.pack("!BBHHH", 8, 0, checksum, pkt_id, seq)
        packet = header + data
        try:
            sock.sendto(packet, (ip, 0))
            recv, _ = sock.recvfrom(1024)
            # ICMP reply type 在 IP 头(20字节)后的第0字节
            icmp_type = recv[20]
            return icmp_type == 0  # 0 = echo reply
    ...
    def _tcp_ping(self, ip: str) -> bool:
        """无 root 权限时的备用存活检测：尝试连接 80/443 端口"""
        for port in (80, 443):
