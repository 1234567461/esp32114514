...
OpenClaw 通过 HTTP 调用本桥接服务的工具接口，桥接再转发给 ESP32 或
调用本地网络扫描器。

工具列表（均为合法功能）:
  - robot_move        控制机器人移动
  - robot_arm         控制机械臂角度
  - robot_status      获取机器人状态
  - network_scan      扫描指定 IP 的端口
  - network_discover  发现子网存活主机
  - network_ping      ping 连通性测试
  - network_traceroute 路由追踪
  - wifi_signal       当前 Wi-Fi 信号质量
"""
import json
import os
import requests
